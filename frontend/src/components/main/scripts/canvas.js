...
function clicked(e, r, setChecks, checks) {
    ...
    if (checkNumbers(r_val, -3, 5) && checkNumbers(x, -3, 5) && checkNumbers(y, -5, 3)) {
        let information = {
...
function drawCanvas(context, r, checks) {
    ...
    context.fillStyle = "#bbb2e9";
    context.fillRect(width / 5 * 2.5, height/2, width / 5*2, height / 5 * 2);
    context.beginPath();
    context.moveTo(width / 2, height / 2);
    context.lineTo(width / 5 * 1.5, height / 5 * 2.5);
    ...
    context.strokeText("Y", width / 1.97, height / 22);
    context.strokeText("X", width / 50 * 49, height / 2.05);
